...
{
    public sealed class FusedMatMulAddRelu : Operation
    {
        public FusedMatMulAddRelu(ImmutableList<IExpression> arguments) : base(arguments) { }
        public FusedMatMulAddRelu(IExpression a, IExpression b, IExpression c) : base(ImmutableList.Create(a, b, c)) { }
        ...
        public override IExpression Canonicalize()
        {
            ...
            return new FusedMatMulAddRelu(args);
        }

        public override string ToDisplayString() => $"FusedMatMulAddRelu({string.Join(", ", Arguments.Select(a => a.ToDisplayString()))})";
        public override bool InternalEquals(IExpression other) => other is FusedMatMulAddRelu f && ExpressionHelpers.SequencesInternalEquals(Arguments, f.Arguments);
        public override int InternalGetHashCode() => ExpressionHelpers.GetSequenceHashCode(GetType(), Arguments);
        public override Operation WithArguments(ImmutableList<IExpression> newArgs) => new FusedMatMulAddRelu(newArgs);
    }
}
